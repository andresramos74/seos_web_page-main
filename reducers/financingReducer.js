import { TYPES_FORM_FINANCING } from "actions/financingActions";
import { TOUCHED_STATES } from "helpers/states";
import { validateCity, validateCompanyName, validateEmail, validateKwInstalled, validateLead, validateNit, validateNumberClients, validatePhoneNumber, validateWebPage, validateDate } from "helpers/validationsFinancing";
import { formatNit, generateVerificationDigit } from 'helpers/financingHelpers'
...
const initialValuesFinancing = {
  ...
  touched: {
    ...
    numero_cuotas: TOUCHED_STATES.NOT_TOUCHED,
  }
}

function financingReducer(state, action) {
  switch (action.type) {
    case (TYPES_FORM_FINANCING.CHANGE_COMPANY_NAME): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_NIT): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_DATE_OF_FUNDATION): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_CITY_CONSTITUTION): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_KW_INSTALLED): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_NUMBER_CLIENTES): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_ACOSOL_ASSOCIATE): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_INFLUENCY_CITY): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_EMAIL): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_PHONE_NUMBER): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_WEB_PAGE): {
    ...
    case (TYPES_FORM_FINANCING.CHANGE_NUMERO_CUOTAS): {

      //* Validate the selection of the input
      let error = validateLead(action.payload)

      //* Delete the previous errors
      if (!error && state.errors.numero_cuotas) {
        delete state.errors.numero_cuotas
      }

      //* Update the state
      return error ? {
        values: { ...state.values, numero_cuotas: action.payload },
        errors: { ...state.errors, numero_cuotas: error },
        touched: { ...state.touched, numero_cuotas: TOUCHED_STATES.TOUCHED_FIRST_TIME }
      } : {
        values: { ...state.values, numero_cuotas: action.payload },
        errors: { ...state.errors },
        touched: { ...state.touched, numero_cuotas: TOUCHED_STATES.TOUCHED_OK }
      }
...
export { initialValuesFinancing, financingReducer }