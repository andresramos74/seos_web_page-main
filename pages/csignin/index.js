...
const Partners = () => {
    ...
    const formatObjectToSend = async (values) => { // * Create a new object with all values of the form
        ...
        let nit = `${
            ...
            newValues.nit.verificationDigit
        }`;

        // * Format of the date that the input give YYYY/MM/DDnpm ci
    ...
    if (data) {
        return (<section className="font-['Poppins'] overflow-x-hidden flex flex-col h-screen w-screen">
            <header className="sticky top-0 w-full bg-seos-blue-dark">
                <Head>
                    <title>SEOS Energy | Partners</title>
        ...
        return (<section className="font-['Poppins'] overflow-x-hidden flex flex-col h-screen w-screen">
            ...
            <article className="grow flex items-center">
                <p className="w-2/4 h-min py-7 px-4 m-auto text-center font-semibold bg-slay-bg">
                    Hemos encontrado un usuario con el mismo número de NIT, por favor
                                                                    verifica tus datos, si tu problema persiste por favor contacta
                                                                    soporte técnico
                </p>
            </article>
        </section>);
    ...
    return (<section className="flex flex-col font-['Poppins'] w-full overflow-x-hidden">
        <header className="sticky top-0 w-full bg-seos-blue-dark">
            <Head>
                <title>SEOS Energy | Partners</title>
                <meta name="description" content="SEOS Energy es una compañía enfocada a....."/>
        ...
        <form className="flex flex-col pb-4 m-auto mt-8 mb-32 w-full border border-gray sm:w-4/5 md:w-[680px] lg:w-[750px] h-min rounded-md"
            onSubmit={handleSubmit}>
            <p className="px-3 py-2 border-b border-b-gray text-lg font-semibold">
                Afiliación cliente SEOS
            </p>
            <div className="flex flex-wrap">
                <PartnersLegalData values={values}
                    errors={errors}
                    touched={touched}
                    dispatch={dispatch}/>
                <PartnersCompanyData values={values}
                    errors={errors}
                    touched={touched}
                    dispatch={dispatch}/>
            </div>
            <PartnersContactData values={values}
                errors={errors}
                touched={touched}
            ...
            <article className="mt-4 px-4 py-2">
                ...
                <section className="w-full my-2 mt-4"> {
                    ...
                    <button className="flex mx-auto mt-2 px-4 h-8 leading-8 bg-seos-blue-light text-white hover:bg-seos-blue-dark rounded-md"
