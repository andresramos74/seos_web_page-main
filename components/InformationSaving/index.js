...
const getwattage = (selectedMethod, informationLead) => {
  ...
  const invoice_value_unformat = informationLead.invoice_value
    .substring(2, informationLead.invoice_value.length)
    .replaceAll(".", "");

  //* If the stratum is greatest than four is neccesary
  const invoice_value =
    stratum > 4
      ? Number(invoice_value_unformat) / 1.2
      : Number(invoice_value_unformat);

  //* Get the value of the radiation depending on the department
  const value_radiation = VALUES_RADIATION.find(
    (department) => department.department === informationLead.department
  );

  //* Get the price of the kWh depending on the department
  const preciokwh = PRICE_KWH.find(
    (department) => department.department === informationLead.department
  );
...
const getNumberOfBatteries = (selectedMethod, informationLead) => {
  ...
  const preciokwh = PRICE_KWH.filter(
    (department) => department.department === informationLead.department
  );
  //* Use the formula to calculate wattage per hour
  const necessaryWattageInHoursWithoutElectricity =
    (invoice_value_unformat / preciokwh[0].value / 30 / 24) *
    hours_without_electricity;
  const batteriesNecessary =
    necessaryWattageInHoursWithoutElectricity / wattageLuziol;
...
const InformationSaving = ({
  ...
  const [isLowerOfTheNecessaryValue, setIsLowerOfTheNecessaryValue] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const savingOfTheProject = getSaving(
      wattageToDisplay,
      informationLead,
      cost
    )?.toString();
    const numberOfLuzioles = getNumberOfBatteries(
      selectedMethod,
      informationLead
    )?.toString();
    const valueBatteries = getCostBatteries(
      selectedMethod,
      numberOfLuzioles
    )?.toString();
    ...
    const savingWithFormat = new Intl.NumberFormat("es-CO").format(
      savingOfTheProject
    );
    const valueBatteriesWithFormat = new Intl.NumberFormat("es-CO").format(
      valueBatteries
    );
  ...
  const handleSubmit1 = (event) => {
    window.location.href =
      "https://api.whatsapp.com/send?phone=573103865380&text=Mi proyecto aún no es rentable, mi correo electrónico es: ";
  };

  const handleSubmit2 = (event) => {
    window.location.href =
      "https://api.whatsapp.com/send?phone=573103865380&text=Mi proyecto excede los 10 kWp, ¿me podrían ayudar con una asesoría al respecto?";
  };

  const handleSubmit3 = (event) => {
    window.location.href =
      "https://api.whatsapp.com/send?phone=573103865380&text=Estoy interesado en un sistema para olvidarme de los cortes de energía. 🔋";
  };
        ...
        {selectedMethod === METHODS_OF_SAVING.SOLAR && (
          <ul className="list-disc list-outside">
            <li className="mb-1">
              ...
              <p className="text-red-warning inline">{wattage} kWp</p>
            </li>
            <li className="mb-1">
              Número de paneles solares:{" "}
              <p className="text-red-warning inline">
                {" "}
                {Number(wattage) / 0.5} paneles
              </p>
            </li>
            <li className="mb-1">
              Número de inversores para tu proyector:{" "}
              <p className="text-red-warning inline"> 1 inversor</p>
            </li>
            <li className="mb-1">
              Ahorro anual de tu proyecto:{" "}
              <p className="text-red-warning inline"> ${saving}</p>
            </li>
            <li className="mb-1">
              Costo de tu proyecto:{" "}
              <p className="text-red-warning inline"> ${projectCost}</p>
            </li>
          </ul>
        )}
        ...
        {selectedMethod === METHODS_OF_SAVING.SYSTEM_SUPPORT && (
          <ul className="list-disc list-outsid">
            <li className="mb-1">
              ...
              <p className="text-red-warning inline"> {luzioles} unidades</p>
            </li>
            <li className="mb-1">
              Costo de las baterías:{" "}
              <p className="text-red-warning inline"> ${costBatteries}</p>
            </li>
          </ul>
        )}
        {selectedMethod === METHODS_OF_SAVING.SUPPORT_SOLAR && (
