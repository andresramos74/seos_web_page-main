        ...
        <form className='flex flex-col h-full w-80 sm:w-[48%] pr-2 md:px-8 pt-4 pb-7 mr-2 border border-gray rounded-md shadow-md' data-cy="form">
            ...
            <section className='relative h-8 mt-8'>
                <label id="invoice_value">
                    <span className='content absolute transition duration-100 bottom-0 z-10 p-1 pl-2 w-full'
                        onClick={changeClassSpan}>Valor de tu factura:</span>
                    <input name="invoice_value" className='w-full bottom-0 outline-none border border-gray focus:border-seos-blue-light rounded-[4px] p-1 pl-1'
            ...
            {
            errors ?. invoice_value || (touched.invoice_value === TOUCHED_STATES.TOUCHED_FIRST_TIME) ? <div className={`text-red-warning mt-1 text-sm mb-8`}
            ...
            <section className='relative h-8'>
                ...
                <label id="stratum" className="absolute h-full w-full">
                    <span className='content absolute transition duration-300 bottom-0 z-10 p-1 pl-2 w-full'
                        onClick={changeClassSpan}>Estrato:</span>
                    <input name="stratum" className='w-full bottom-0 outline-none border border-gray focus:border-seos-blue-light rounded-[4px] p-1 pl-1'
                        onFocus={changeClassSpan}
                        onBlur={handleBlurField}
                        onChange={handleChange}
                        value={
            ...
            {
            errors ?. stratum || (touched.stratum === TOUCHED_STATES.TOUCHED_FIRST_TIME) ? <div className={`mb-8 text-red-warning text-sm`}
            ...
            {
            (selectedMethod === METHODS_OF_SAVING.SUPPORT_SOLAR || selectedMethod === METHODS_OF_SAVING.SYSTEM_SUPPORT) && <section className='relative h-8'>
                <label id="hours_without_electricity" className="absolute h-full w-full">
                    <span className='content absolute transition duration-300 bottom-0 z-10 p-1 pl-2 w-full'
                        onClick={changeClassSpan}>Horas sin electricidad:</span>
                    <input name="hours_without_electricity" className='w-full bottom-0 outline-none border border-gray focus:border-seos-blue-light rounded-[4px] p-1 pl-1'
                        onFocus={changeClassSpan}
                        onBlur={handleBlurField}
                        onChange={handleChange}
                        values={
                            values.hours_without_electricity
                        }
            ...
            {
            ((errors ?. hours_without_electricity || (touched.hours_without_electricity === TOUCHED_STATES.TOUCHED_FIRST_TIME)) && (selectedMethod === METHODS_OF_SAVING.SUPPORT_SOLAR || selectedMethod === METHODS_OF_SAVING.SYSTEM_SUPPORT)) ? <div className='text-red-warning mb-6 text-sm' data-cy="error-hours_without_electricity">
                {
                errors.hours_without_electricity
            }</div> : <p className={
                `${
                    !(selectedMethod === METHODS_OF_SAVING.SUPPORT_SOLAR || selectedMethod === METHODS_OF_SAVING.SYSTEM_SUPPORT) && "hidden"
                } mb-9`
            }></p>
        }
            <SelectInformationCO goToForm={goToForm}
                values={values}
                errors={errors}
                touched={touched}
                dispatch={dispatch}/>
            <section className='mb-4'>
                ...
                <input type='checkbox' id='rural' name='rural' value='Si' className='ml-2'
                    onChange={
                        () => {
            ...
            <input type='submit' className='transition w-min m-auto px-2 py-1 bg-seos-blue-light rounded-md text-white cursor-pointer hover:bg-seos-blue-dark' value="Confirmar datos" form="{}"
