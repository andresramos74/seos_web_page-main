...
const PartnersLegalData = ({
    ...
    useEffect(() => { // ! When the component is render for the first time data is null, for this reason when is null return
        if (data === null) 
            return
    ...
    const handleClickDataListOption = (event, fieldName) => {
        ...
        if (!event.target.classList.contains('react-datalist-input__listbox-option')) 
            return
    ...
    return (<article className='px-3 py-4 md:w-2/4 w-full border-b border-b-gray md:border-r md:border-r-gray md:border-b-0'>
        <h1 className='font-semibold'>Información de Usuario</h1>
        <section className='relative h-8 mt-7'>
            <label id="company_name">
                <span className='content absolute transition duration-300 bottom-0 z-10 p-1 pl-2 w-full'
                    onClick={changeClassSpan}>Nombre completo:</span>
                <input name="company_name" className='w-full bottom-0 outline-none border border-gray focus:border-seos-blue-light rounded-[4px] p-1 pl-1'
                    onFocus={changeClassSpan}
                    onBlur={
                        (e) => handleBlurInput(e, "company_name")
                    }
                    onChange={
                        (e) => handleChange(e, "company_name")
                    }
                    value={
                        values.company_name
                    }
        ...
        {
        (errors.company_name || touched.company_name === TOUCHED_STATES.TOUCHED_FIRST_TIME) ? <div className='text-red-warning mb-8 px-1 text-sm'> {
            errors.company_name
        }</div> : <p className='mb-8'></p>
    }
        <section className='relative flex h-8'>
            <label id="nit" className="w-[85%]">
                <span className='content absolute transition duration-300 bottom-0 z-10 p-1 pl-2'
                    onClick={changeClassSpan}>NIT de tu compañía:</span>
                <input name="nit" className='outline-none w-full border border-gray focus:border-seos-blue-light rounded-[3px] p-1 pl-1'
                    onFocus={changeClassSpan}
                    onBlur={
                        (e) => handleBlurInput(e, "nit")
                    }
                    onChange={
                        (e) => handleChange(e, "nit")
                    }
                    value={
                        values.nit.value
                    }
            ...
            <p className='mx-auto leading-[2.3rem]'>
                -
            </p>
            <input type="text" className="outline-none w-[10%] rounded-[3px] p-1 border bg-gray text-center"
                value={
                    values.nit.verificationDigit
                }
                disabled/>
        </section>
        {
        (errors.nit || touched.nit === TOUCHED_STATES.TOUCHED_FIRST_TIME) ? <div className='text-red-warning mb-8 px-1 text-sm'> {
            errors.nit
        }</div> : <p className='mb-8'></p>
    }
        <section className='relative h-8 mt-4'>
            <label id='date'>
                <span className='content text-black absolute w-full transition duration-300 bottom-0 z-10 p-1 pl-2'
                    onClick={changeClassSpan}>Fecha de constitución:</span>
                <input type="date" name="date"
                    className={
                        `outline-none w-full h-8 border text-black ${
                            spanActive === TOUCHED_STATES.NOT_TOUCHED ? "text-opacity-0" : "text-opacity-100"
                        } border-gray focus:border-seos-blue-light rounded-[3px] p-1 pl-1`
                    }
                    min="1950-01-01"
                    max={actualDate}
        ...
        {
        (errors.date || touched.date === TOUCHED_STATES.TOUCHED_FIRST_TIME) ? <div className='text-red-warning mb-8 px-1 pt-[0.25px] text-sm'> {
            errors.date
        }</div> : <p className='mb-8'></p>
    }
        <section> {
            municipalities && <DatalistInput label="Ciudad de constitución:"
                onSelect={
                ...
                onChange={
                    (e) => handleChange(e, 'city_constitution')
                }
                onBlur={
                    (e) => handleBlurDataList(e, 'city_constitution')
                }
                onClick={
                    (e) => handleClickDataListOption(e, 'city_constitution')
                }
                inputProps={
                    {
        ...
        <section>
            <div class="mb-3 pt-4">
                <label for="formFileSm" class="form-label">Fotocopia de Cédula de ciudadanía (PDF):</label>
                <input class="form-control form-control-sm" id="formFileSm" type="file"/>
            </div>
            <div class="mb-3 pt-4">
                <label for="formFileSm" class="form-label">Certificado laboral (PDF):</label>
                <input class="form-control form-control-sm" id="formFileSm" type="file"/>
            </div>
            <div class="mb-3 pt-4">
                <label for="formFileSm" class="form-label">Extractos bancarios de los 3 últimos meses (PDF):</label>
                <input class="form-control form-control-sm" id="formFileSm" type="file"/>
            </div>
        </section>
        {
        (errors.city_constitution || touched.city_constitution === TOUCHED_STATES.TOUCHED_FIRST_TIME) ? <div className='text-red-warning mb-2 px-1 text-sm'> {
            errors.city_constitution
